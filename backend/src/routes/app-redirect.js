...
// Weekly setups deep link
router.get('/setups/weekly', (req, res) => {
  const userAgent = req.get('User-Agent') || '';
  const isMobile = /Android|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(userAgent);

  if (isMobile) {
    res.send(generateSetupsRedirectHTML('weekly'));
  } else {
    res.send(generateSetupsDesktopHTML('weekly'));
  }
});

// Daily setups deep link
router.get('/setups/daily', (req, res) => {
  const userAgent = req.get('User-Agent') || '';
  const isMobile = /Android|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(userAgent);

  if (isMobile) {
    res.send(generateSetupsRedirectHTML('daily'));
  } else {
    res.send(generateSetupsDesktopHTML('daily'));
  }
});
...
function generateSetupsRedirectHTML(alertType) {
  const title = alertType === 'weekly' ? 'Weekly Setups Ready!' : 'Daily Analysis Ready!';
  const description = alertType === 'weekly'
    ? 'Your weekly swing setups are ready to view!'
    : 'Your daily news stocks analysis is ready!';
...
<body>
  ...
  <script>
    // Try to open the app using custom scheme first
    window.location.href = 'logdhan://app/setups/${alertType}';

    // If that doesn't work, try with logdhanapp scheme
    setTimeout(() => {
      window.location.href = 'logdhanapp://setups/${alertType}';
    }, 1000);
...
function generateSetupsDesktopHTML(alertType) {
  const title = alertType === 'weekly' ? 'Weekly Setups Ready!' : 'Daily Analysis Ready!';
  const description = alertType === 'weekly'
    ? 'Your weekly swing setups are ready to view. Download the LogDhan mobile app to see your results:'
    : 'Your daily news stocks analysis is ready. Download the LogDhan mobile app to see your results:';
