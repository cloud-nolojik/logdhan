...
async function sendBulkAnalysisCompletionNotification(session) {
  try {
    ...
    logger.error(`❌ Failed to send bulk analysis completion notification for session ${session.session_id}:`, notificationError);
...
async function getUserWatchlist(userId) {
  try {
    ...
    if (!user || !user.watchlist) {
      ////logger.info((`📊 No watchlist found for user ${userId}`);
      return [];
    }
    ...
    if (!stack.includes('/status')) {
      ...
      ////logger.info((`📊 Loaded ${watchlist.length} stocks from user's watchlist`);
    }return watchlist;
  } catch (error) {
    logger.error('❌ Error loading user watchlist:', error);
    return [];
  }
}
...
export const analyzeAll = async (req, res) => {
  try {
    ...
    //logger.info(`🚀 [ANALYZE-ALL] Mobile app called analyze-all: userId=${userId}, type=${analysis_type}, resume=${resume}`);

    // Delete ALL existing sessions for this user - no conditions, always start fresh
    //logger.info(`🗑️ [ANALYZE-ALL] Deleting all existing sessions for user...`);
    const sessionDeleteResult = await AnalysisSession.deleteMany({
      user_id: userId,
      analysis_type: analysis_type
    });
    //logger.info(`🗑️ [ANALYZE-ALL] Deleted ${sessionDeleteResult.deletedCount} existing sessions`);
    ...
    //     logger.error('Error checking analysis permission:', subscriptionError);
    ...
    //logger.info(`✅ [SESSION CREATED] New session: ${session.session_id} for ${watchlistStocks.length} stocks, userId=${userId}`);
    ...
    await session.save();
    //logger.info(`✅ [SESSION STATUS] Session fully saved to database as 'running': ${session.session_id}`);

    // Double-check by reloading from database
    const verifySession = await AnalysisSession.findById(session._id);
    //logger.info(`🔍 [SESSION VERIFY] Database verification - status: ${verifySession?.status}, id: ${verifySession?.session_id}`);

    // Start background processing IMMEDIATELY after database save
    //logger.info(`🚀 [BACKGROUND] Starting background processing for verified session: ${session.session_id}, status: ${session.status}`);

    // Start processing immediately
    setImmediate(async () => {
      try {
        //logger.info(`🔥 [BACKGROUND] Background processing starting for: ${session.session_id}`);
        await processSessionBasedBulkAnalysis(session);
        //logger.info(`✅ [BACKGROUND] Background processing completed for: ${session.session_id}`);
      } catch (error) {
        logger.error(`❌ [BACKGROUND] Background processing failed for ${session.session_id}:`, error);
        try {
          ...
          if (failedSession) {
            ...
            //logger.info(`💀 [BACKGROUND] Marked session as failed: ${session.session_id}`);
          }
        } catch (saveError) {
          logger.error(`❌ [BACKGROUND] Failed to mark session as failed: ${saveError.message}`);
        }
      }
    });
    ...
    //logger.info(`📤 [ANALYZE-ALL RESPONSE] Returning session_id=${responseData.session_id}, status=${responseData.status}, total_stocks=${responseData.total_stocks}`);
    ...
    //logger.info("bulk analysis session response:", JSON.stringify(finalResponse));
...
export const cancelAnalysis = async (req, res) => {
  try {
    ...
    ////logger.info((`🛑 Cancelling bulk analysis for user ${userId}, type: ${analysis_type}`);
    ...
    ////logger.info((`✅ Cancelled session: ${session.session_id}`);
...
export const getStatus = async (req, res) => {
  try {
    ...
    //logger.info(`📋 [SESSION SEARCH] Single session found: ${activeSession ? `${activeSession.session_id} (${activeSession.status})` : 'null'}`);
    ...
    if (activeSession) {
      ...
      const stockDetails = await Promise.all(activeSession.metadata.watchlist_stocks.map(async (stock) => {
            ...
            try {
              ...
              try {
                ...
                if (isNaN(sessionDate.getTime())) {
                  ...
                  logger.warn(`⚠️ Invalid session date for ${stock.trading_symbol}, using fallback date`);
                }
              } catch (dateError) {
                sessionDate = new Date(Date.now() - 60 * 60 * 1000); // 1 hour ago
                logger.warn(`⚠️ Date parsing error for ${stock.trading_symbol}, using fallback date:`, dateError.message);
              }
              ...
              logger.warn(`⚠️ Could not check analysis status for ${stock.trading_symbol}:`, analysisCheckError.message);
        ...
        if (stockStatus === 'completed') {
          try {
            ...
            logger.warn(`⚠️ Failed to fetch strategy for ${stock.trading_symbol}:`, strategyError.message);
          }
        }
      ...
      //logger.info("bulk analysis status response:", JSON.stringify({
      //     success: true,
    ...
    logger.error('❌ Error getting bulk analysis status:', error);
    res.status(500).json({
      success: false,
...
export const getStrategies = async (req, res) => {
  try {
    ...
    ////logger.info((`📊 Fetching strategies for user ${userId}, type: ${analysis_type}`);
    ...
    try {
      ...
      logger.error('❌ Error converting userId to ObjectId:', error.message);
      userObjectId = userId;
    }
    ...
    ////logger.info((`📊 Found ${analyses.length} analyses (completed + failed)`);
    ...
    ////logger.info((`📊 Sorted and limited to ${limitedStrategies.length} strategies from ${allStrategies.length} total`);
...
// reanalyzeStock
export const reanalyzeStock = async (req, res) => {
  try {
    const userId = req.user.id;
    ...
    ////logger.info((`🔄 Reanalyzing stock ${stock_symbol} for user ${userId}`);
    ...
    try {
      ...
      logger.error(`❌ Price fetch failed for ${stock_symbol}:`, priceError.message);
      return res.status(400).json({
        success: false,
    ...
    try {
      ...
      if (result.success) {
        ////logger.info((`✅ Reanalysis completed for ${stock_symbol}`);
        res.status(200).json({
          success: true,
        ...
        logger.error(`❌ Reanalysis failed for ${stock_symbol}:`, result.message);
        res.status(500).json({
          success: false,
      ...
      logger.error(`❌ Reanalysis error for ${stock_symbol}:`, analysisError.message);
      res.status(500).json({
        success: false,
    ...
    logger.error('❌ Error in reanalyze-stock:', error);
    res.status(500).json({
      success: false,
      error: 'Server error',
      message: error.message
...
export const debugSessions = async (req, res) => {
  try {
    ...
    logger.error('❌ Error in debug sessions:', error);
    res.status(500).json({
      success: false,
...
// timingCheck
export const timingCheck = async (req, res) => {
  try {
    const analysisPermission = await StockAnalysis.isBulkAnalysisAllowed();
    ...
    logger.error('❌ Error checking bulk analysis timing:', error);
    res.status(500).json({
      success: false,
...
export const recordOrderPlacement = async (req, res) => {
  try {
    const userId = req.user.id;
    const {
      analysisId,
    ...
    logger.error('❌ Error recording order placement:', error);
    res.status(500).json({
      success: false,
...
//     ////logger.info((`🔄 [BULK ANALYSIS] Starting background processing for user ${userId} with ${stocks.length} stocks, type: ${analysisType}`);
//     let completed = 0;
//     let failed = 0;
...
//         ////logger.info((`📦 [BULK ANALYSIS] Processing batch ${i + 1}/${totalBatches} (stocks ${batchStart + 1}-${batchEnd})`);

//         // Process batch in parallel
//         const batchPromises = batch.map(async (stock, index) => {
//             try {
//                 ////logger.info((`🔍 [BULK ANALYSIS] Analyzing ${stock.trading_symbol} (${stock.name})`);
...
//                     ////logger.info((`💰 [BULK ANALYSIS] Price fetched for ${stock.trading_symbol}: ₹${currentPrice}`);
//                 } catch (priceError) {
//                     logger.error(`❌ [BULK ANALYSIS] Price fetch failed for ${stock.trading_symbol}:`, priceError.message);
...
//                             }
//                         );
//                         ////logger.info((`📝 Created failed analysis record for ${stock.trading_symbol} (price fetch failed)`);
//                     } catch (saveError) {
//                         logger.error(`❌ Failed to save failed analysis record for ${stock.trading_symbol}:`, saveError.message);
//                     }
...
//                     ////logger.info((`⚠️ [BULK ANALYSIS] Invalid price for ${stock.trading_symbol} (${currentPrice}), skipping`);
//                     failed++;
...
//                         }
//                     );
//                         ////logger.info((`📝 Created failed analysis record for ${stock.trading_symbol} (invalid price)`);
//                     } catch (saveError) {
//                         logger.error(`❌ Failed to save failed analysis for invalid price ${stock.trading_symbol}:`, saveError.message);
//                     }
...
//                         logger.info(`⚖️ [BULK ANALYSIS] Daily AI limit reached for user ${userId} after ${used}/${limit} stocks`);
//                         return; // Stop processing more stocks for this user
//                     }
//                     // Other limit reasons (e.g., watchlist quota)
//                     logger.info(`⚠️ [BULK ANALYSIS] Skipping ${stock.trading_symbol} - ${limitsCheck.reason}: ${limitsCheck.message}`);
...
//                 completed++;
//                 ////logger.info((`✅ [BULK ANALYSIS] Completed ${stock.trading_symbol} (${completed}/${stocks.length})`);

//             } catch (error) {
//                 failed++;
//                 logger.error(`❌ [BULK ANALYSIS] Failed to analyze ${stock.trading_symbol}:`, error.message);
...
//                         }
//                     );
//                     ////logger.info((`📝 Created failed analysis record for ${stock.trading_symbol}: ${failedAnalysis._id}`);
//                 } catch (saveError) {
//                     logger.error(`❌ Failed to save failed analysis record for ${stock.trading_symbol}:`, saveError.message);
//                 }
//             }
...
//         if (dailyLimitReached) {
//             logger.info(`⚖️ [BULK ANALYSIS] Stopping further batches for user ${userId} after hitting daily limit`);
//             break;
//         }
...
//             ////logger.info((`⏱️ [BULK ANALYSIS] Waiting ${delayMs/1000}s before next batch to avoid rate limiting...`);
//             await new Promise(resolve => setTimeout(resolve, delayMs));
//         }
//     }

//     ////logger.info((`🏁 [BULK ANALYSIS] Completed for user ${userId}. Success: ${completed}, Failed: ${failed}, Total processed: ${completed + failed}/${stocks.length}`);
// }
...
async function processSessionBasedBulkAnalysis(session) {
  //logger.info(`🔄 [SESSION ANALYSIS] Starting session-based processing: ${session.session_id}`);

  try {
    // Session status already updated to "running" before this function is called
    //logger.info(`📝 [SESSION ANALYSIS] Session already running, starting stock processing: ${session.session_id}`);

    // Debug: Log the full session metadata
    //logger.info(`🔍 [SESSION DEBUG] Session metadata:`, JSON.stringify(session.metadata, null, 2));
    //logger.info(`🔍 [SESSION DEBUG] Session watchlist_stocks count: ${session.metadata?.watchlist_stocks?.length || 0}`);
    ...
    //logger.info(`📊 [SESSION ANALYSIS] Unprocessed stock details:`, unprocessedStocks.map(s => ({ 
    ...
    if (unprocessedStocks.length === 0) {
      //logger.info(`✅ [SESSION ANALYSIS] All stocks already processed for session: ${session.session_id}`);
    ...
    //logger.info(`📋 [SESSION ANALYSIS] Processing ${unprocessedStocks.length} remaining stocks`);
    ...
    for (let i = 0; i < unprocessedStocks.length; i++) {
      ...
      if (!refreshedSession) {
        //logger.info(`❌ [SESSION ANALYSIS] Session not found: ${session.session_id} - stopping processing`);
        return;
      }

      if (refreshedSession.status === 'cancelled') {
        //logger.info(`🛑 [SESSION ANALYSIS] Session cancelled: ${session.session_id} - stopping processing`);
        return;
      }
      ...
      //logger.info(`🔍 [SESSION ANALYSIS] Processing stock ${i + 1}/${unprocessedStocks.length}: ${stock.trading_symbol} (${stock.instrument_key})`);
      ...
      try {
        ...
        if (freshSession) {
          ...
          if (stockInSession) {
            ...
            //logger.info(`📝 [SESSION ANALYSIS] Marked ${stock.trading_symbol} as processing started`);
          } else {

            //logger.info(`⚠️ [SESSION ANALYSIS] Could not find ${stock.trading_symbol} in session stocks`);
          }
          await freshSession.save();
          //logger.info(`✅ [SESSION ANALYSIS] Updated session with current stock: ${stock.trading_symbol}`);

          // Update local session reference
          Object.assign(session, freshSession.toObject());
        }
      } catch (sessionUpdateError) {
        logger.error(`⚠️ [SESSION ANALYSIS] Failed to update session start for ${stock.trading_symbol}:`, sessionUpdateError.message);
        // Continue processing even if session update fails
      }

      //logger.info(`✅ [SESSION ANALYSIS] Valid price ₹${numericPrice} for ${stock.trading_symbol}, starting AI analysis...`);
      ...
      try {
        //logger.info(`🤖 [SESSION ANALYSIS] Calling onDemandAnalysisService.analyze for ${stock.trading_symbol}...`);
        ...
        //logger.info(`🎯 [SESSION ANALYSIS] Analysis completed for ${stock.trading_symbol}, result:`, analysisResult?.success ? 'SUCCESS' : 'FAILED');

        if (analysisResult && analysisResult.success) {
          // Mark stock as successfully processed (includes quick reject as success)
          await markStockAsSuccessful(session, stock);
          //logger.info(`✅ [SESSION ANALYSIS] Successfully completed ${stock.trading_symbol} (quickReject: ${analysisResult.fromQuickReject || false})`);
          ...
          //logger.info(`❌ [SESSION ANALYSIS] Analysis failed for ${stock.trading_symbol}: ${errorMsg}`);
          await markStockAsFailed(session, stock, errorMsg);
        }

      } catch (analysisError) {
        logger.error(`❌ [SESSION ANALYSIS] Analysis error for ${stock.trading_symbol}:`, analysisError.message);
      ...
      //logger.info(`⏱️ [SESSION ANALYSIS] Waiting ${delayBetweenStocks/1000}s before next stock to prevent rate limiting...`);
      await new Promise((resolve) => setTimeout(resolve, delayBetweenStocks));
    }

    // Mark session as completed
    //logger.info(`🏁 [SESSION ANALYSIS] All stocks processed, marking session as completed`);
    session.status = 'completed';
    session.completed_at = new Date();
    session.current_stock_key = null;
    await session.save();

    //logger.info(`🏁 [SESSION ANALYSIS] Session completed: ${session.session_id}. Success: ${session.successful_stocks}, Failed: ${session.failed_stocks}`);

    // Send WhatsApp notification for bulk analysis completion
    await sendBulkAnalysisCompletionNotification(session);

  } catch (sessionError) {
    logger.error(`❌ [SESSION ANALYSIS] Session failed: ${session.session_id}`, sessionError);
    logger.error(`❌ [SESSION ANALYSIS] Full error details:`, sessionError.stack);
    ...
    try {
      ...
      session.completed_at = new Date();
      await session.save();
      //logger.info(`✅ [SESSION ANALYSIS] Marked session as failed: ${session.session_id}`);
    } catch (saveError) {
      logger.error(`❌ [SESSION ANALYSIS] Failed to save failed session: ${session.session_id}`, saveError);
    }
  }
}
...
async function markStockAsFailed(session, stock, errorReason, retryCount = 0) {
  ...
  try {
    ...
    const freshSession = await AnalysisSession.findById(session._id);
    if (!freshSession) {
      logger.error(`❌ Session not found for update: ${session.session_id}`);
      return;
    }
    ...
    if (sessionError.name === 'VersionError' && retryCount < maxRetries) {
      ////logger.info((`🔄 Retrying session update for ${stock.trading_symbol} (attempt ${retryCount + 1}/${maxRetries})`);
      ...
      logger.error(`❌ Failed to update session for ${stock.trading_symbol} after ${retryCount + 1} attempts:`, sessionError.message);
      throw sessionError;
    }
  }
}
