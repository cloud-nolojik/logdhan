...
// API response time logger — highlights slow requests
app.use((req, res, next) => {
  ...
  res.on('finish', () => {
    ...
    const icon = rounded > 100 ? '🐢' : rounded > 30 ? '🟡' : '⚡';
    console.log(`${icon} ${req.method} ${req.originalUrl} ${res.statusCode} — ${rounded}ms`);
  });
