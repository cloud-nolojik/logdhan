...
pendingBracketOrderSchema.statics = {
    ...
    async createPendingOrder(orderData) {
        try {
            ...
            console.log(`📝 Created persistent pending bracket order for ${orderData.order_id}`);
            return pendingOrder;
        } catch (error) {
    ...
    async findAndMarkProcessing(orderId) {
        try {
            ...
            if (pendingOrder) {
                console.log(`📊 Found pending bracket order for ${orderId}, attempt #${pendingOrder.processing_attempts}`);
            }
    ...
    async markProcessed(orderId, bracketOrderIds) {
        try {
            ...
            if (result) {
                console.log(`✅ Marked bracket order ${orderId} as processed`);
            }
    ...
    async markFailed(orderId, errorMessage, errorDetails = null) {
        try {
            ...
            if (result) {
                console.log(`❌ Marked bracket order ${orderId} as failed: ${errorMessage}`);
            }
    ...
    async cleanupExpired() {
        try {
            ...
            if (result.deletedCount > 0) {
                console.log(`🧹 Cleaned up ${result.deletedCount} expired pending bracket orders`);
            }
