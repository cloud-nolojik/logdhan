...
class BatchManager {
    ...
    async createOptimalBatches() {
        try {
            console.log('🔄 [BATCH MANAGER] Creating optimal batches from active subscriptions...');
            ...
            if (activeSubscriptions.length === 0) {
                console.log('📭 [BATCH MANAGER] No active subscriptions found - no batches needed');
                return [];
            }

            console.log(`📊 [BATCH MANAGER] Found ${activeSubscriptions.length} active subscriptions`);
            ...
            console.log(`📈 [BATCH MANAGER] Grouped into ${analysisGroups.size} unique analyses`);
            ...
            console.log('📋 [BATCH MANAGER] Analysis distribution:');
            analysisDistribution.forEach((item, idx) => {
                console.log(`   ${idx + 1}. ${item.stock_symbol} (${item.analysisId.slice(-8)}): ${item.strategies_count} strategies, ${item.total_users} users`);
            });
            ...
            console.log(`✅ [BATCH MANAGER] Created ${batches.length} optimal batches`);
            batches.forEach((batch, idx) => {
                console.log(`   Batch ${batch.batchId}: ${batch.analysisIds.length} analyses, ${batch.estimated_total_strategies} strategies`);
            });
    ...
    splitLargeBatch(largeBatch) {
        ...
        console.log(`📊 [BATCH MANAGER] Split large batch ${largeBatch.batchId} (${largeBatch.analysisIds.length} analyses) into ${splitBatches.length} smaller batches`);
        return splitBatches;
    }
    ...
    updateBatchConfiguration(performanceMetrics) {
        ...
        if (avgProcessingTime > 45000) { // More than 45 seconds
            ...
            console.log(`⚡ [BATCH MANAGER] Reduced batch size to ${this.batchSize} due to high processing time`);
            ...
            console.log(`🚀 [BATCH MANAGER] Increased batch size to ${this.batchSize} due to good performance`);
        }

        // Adjust based on error rate
        if (errorRate > 0.1) { // More than 10% error rate
            this.batchSize = Math.max(10, this.batchSize - 5);
            console.log(`🛡️ [BATCH MANAGER] Reduced batch size to ${this.batchSize} due to high error rate`);
        }
    }
}
